...
class RemoteAPIClient:
    ...
    def _waitForSignal(self, sigName):
        ...
        while True:
            retVal = (
                self.sim.getInt32Signal(sigName) is not None
                or self.sim.getFloatSignal(sigName) is not None
                or self.sim.getDoubleSignal(sigName) is not None
                or self.sim.getStringSignal(sigName) is not None
            )
            if retVal:
                break
            self.step()
    ...
    def _moveToConfig(
        ...
        ruckigObject = self.sim.ruckigPos(
            len(currentPos), 0.0001, flags, currentPosVelAccel, maxVelAccelJerk, sel, targetPosVel
        )
        result = 0
        timeLeft = 0
        while result == 0:
    ...
    def _moveToPose(
        ...
        if usingMatrices:
            ...
            currentMatrix = self.sim.buildMatrixQ(
                currentPoseOrMatrix,
                [
                    currentPoseOrMatrix[3],
                    currentPoseOrMatrix[4],
                    currentPoseOrMatrix[5],
                    currentPoseOrMatrix[6],
                ],
            )
            targetMatrix = self.sim.buildMatrixQ(
                targetPoseOrMatrix,
                [
                    targetPoseOrMatrix[3],
                    targetPoseOrMatrix[4],
                    targetPoseOrMatrix[5],
                    targetPoseOrMatrix[6],
                ],
            )
        ...
        timeLeft = 0
        if metric:
            # Here we treat the movement as a 1 DoF movement, where we simply interpolate via t
            # between the start and goal pose. This always results in straight line movement paths
            dx = [
                (targetMatrix[3] - currentMatrix[3]) * metric[0],
                (targetMatrix[7] - currentMatrix[7]) * metric[1],
                (targetMatrix[11] - currentMatrix[11]) * metric[2],
                angle * metric[3],
            ]
            ...
            if distance > 0.000001:
                ...
                ruckigObject = self.sim.ruckigPos(
                    1, 0.0001, flags, currentPosVelAccel, maxVelAccelJerk, [1], targetPosVel
                )
                result = 0
                while result == 0:
                    dt = timeStep
            ...
            # is handled and controlled individually. This can result in non-straight line movement
            # paths, due to how the Ruckig functions operate depending on 'flags'
            dx = [
                targetMatrix[3] - currentMatrix[3],
                targetMatrix[7] - currentMatrix[7],
                targetMatrix[11] - currentMatrix[11],
                angle,
            ]
            ...
            ruckigObject = self.sim.ruckigPos(
                4, 0.0001, flags, currentPosVelAccel, maxVelAccelJerk, [1, 1, 1, 1], targetPosVel
            )
            result = 0
            while result == 0:
                dt = timeStep
